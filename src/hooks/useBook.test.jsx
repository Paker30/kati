...
describe("useBooks", () => {
  ...
  test("Init hook", () => {
    const wrapper = ({ children }) => (
      <KatiContextProvider>{children}</KatiContextProvider>
    );
  ...
  describe("getAll", () => {
    ...
    test("There was an error", async () => {
      mocks.getAll.mockRejectedValueOnce(
        new Error("Oppps, something has gone wrong!")
      );
      const wrapper = ({ children }) => (
        <KatiContextProvider>{children}</KatiContextProvider>
      );
  ...
  describe.skip("setRead", () => {
    test("Set book as unread", async () => {
      ...
      const { result } = renderHook(() => useBooks(), { wrapper });
      await waitFor(() =>
        result.current.addBook({ ...fakeBook, isRead: true })
      );
    ...
    test("Set book as read", async () => {
      ...
      const { result } = renderHook(() => useBooks(), { wrapper });
      await waitFor(() =>
        result.current.addBook({ ...fakeBook, isRead: false })
      );
      await waitFor(() => result.current.setRead(fakeBook.id));
      expect(mocks.update).toHaveBeenCalledTimes(1);
    });
  ...
  describe("removeBook", () => {
    ...
    test("Removing book fails", async () => {
      ...
      mocks.remove.mockRejectedValueOnce(
        new Error("Oppps something went wrong!")
      );
      const wrapper = ({ children }) => (
        <KatiContextProvider>{children}</KatiContextProvider>
      );
